...
class Metasploit3 < Msf::Auxiliary
	...
	def initialize
		...
		register_options(
			[ OptPath.new('CONFIG_DIR', [true, 'Fingerprints Directory', "#{Msf::Config.install_root}/data/interface_fingerprints/"]),
			OptPath.new('CONFIG', [false, 'Single Fingerprints File; e.g. readme.yml', ""]),
			OptString.new('SINGLE_INTERFACE', [false, 'A single web interface to check for.', ""]),
			OptString.new('DIR', [false, 'To check in a non default directory (e.g. /admin/', ""]),
		], self.class)
		register_advanced_options(
			[OptBool.new('NO_BRUTEFORCE', [false, 'Do not attempt bruteforcing', false]),
		...
		case @response.code
			when 200
				if(check_fingerprint(@response,@interface['fingerprint']))
					...
					# Report the http fingerprint in the database just in case the creds
					#	don't work out
					report_note(
	...
	def check_fingerprint(res,regex)
		...
		if (res.body.to_s =~ pattern)
			print_good("#{target_url}#{@post_to} - Response matched #{pattern} in the body")
			return true
		elsif (res.headers.to_s =~ pattern)
			print_good("#{target_url}#{@post_to} - Response matched #{pattern} in the headers")
			return true
		end
		return false
	end
